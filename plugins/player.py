            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "** ✅انا جاهز لتشغيل الموسيقي...**")

                except UserAlreadyParticipant:
                    pass
                except Exception:
                    await lel.edit(
                        f"**من فضل اصف الحساب المساعد يدويᴛ [المساعد](https://t.me/t.me/RC_JH)** ")
    try:
...
@Client.on_message(commandpro(["/pause", "ايقاف"]) & other_filters)
@errors
...
@Client.on_message(commandpro(["/resume", "استئناف"]) & other_filters)
@errors
@authorized_users_only
...
@Client.on_message(commandpro(["/skip", "تخطي"]) & other_filters)
@errors
@authorized_users_only
async def skip(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("**حالة البوت \n ᴘʟᴀʏɪɴɢ...**")
    else:
...
@Client.on_message(commandpro(["/end", "انهاء"]) & other_filters)
@errors
@authorized_users_only
